...
const Billing = () => {
  ...
  const handlePrint = () => {
    ...
    // Copy all CSS styles and links from the parent window so MUI classes work
    ...
    // Give the browser time to parse the injected CSS before opening print dialog
    setTimeout(() => {
      printWindow.print();
      printWindow.close();
    }, 500);
    ...
    <Box sx={{ p: { xs: 2, md: 4 }, bgcolor: '#f9fafb', minHeight: '100vh' }}>
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' }, mb: 4, gap: 2 }}>
        ...
        {!invoiceGenerated ? (
          <Button 
            ...
            sx={{ bgcolor: '#7A4E2D', '&:hover': { bgcolor: '#3B2C20' }, borderRadius: 2, px: 3, py: 1.2, textTransform: 'none', boxShadow: '0 4px 14px rgba(122, 78, 45, 0.3)' }}
          >
            Generate Invoice
          </Button>
        ) : (
          <Box sx={{ display: 'flex', gap: 2 }}>
            ...
            <Button 
              ...
              sx={{ bgcolor: '#7A4E2D', '&:hover': { bgcolor: '#3B2C20' }, borderRadius: 2, px: 3, textTransform: 'none', boxShadow: '0 4px 14px rgba(122, 78, 45, 0.3)' }}
            >
              Download PDF
            </Button>
          </Box>
      ...
      {invoiceGenerated ? (
        <Card sx={{ maxWidth: 850, margin: '0 auto', borderRadius: 4, boxShadow: '0 12px 40px rgba(0,0,0,0.06)', overflow: 'visible', position: 'relative' }}>
          ...
          <Box sx={{ height: 6, width: '100%', background: 'linear-gradient(90deg, #3B2C20 0%, #7A4E2D 100%)', borderTopLeftRadius: 16, borderTopRightRadius: 16, position: 'absolute', top: 0, left: 0 }} />
          
          <CardContent ref={invoiceRef} sx={{ p: { xs: 3, md: 4 }, pt: { xs: 4, md: 5 } }}>
            {/* Header section */}
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', md: 'center' }, mb: 4, gap: 2 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                <Box sx={{ 
                  width: 48, height: 48, 
                  border: '2px dashed #7A4E2D', 
                  borderRadius: '50%', 
                ...
                <Box>
                  <Typography variant="h5" fontWeight="800" sx={{ color: '#7A4E2D', letterSpacing: '0.05em', lineHeight: 1.2 }}>KACH INN</Typography>
                  <Typography variant="caption" sx={{ color: '#7A4E2D', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600 }}>Luxury Hotel</Typography>
                </Box>
              </Box>
            ...
            <Grid container spacing={3} mb={4}>
              <Grid item xs={12} md={6}>
                <Typography variant="overline" sx={{ color: '#3B2C20', letterSpacing: '0.05em', fontWeight: 600 }}>Bill To</Typography>
              ...
              <Grid item xs={12} md={6}>
                <Box sx={{ bgcolor: '#f9fafb', p: 2, borderRadius: 3 }}>
                  <Typography variant="overline" sx={{ color: '#3B2C20', letterSpacing: '0.05em', fontWeight: 600 }}>Stay Details</Typography>
            ...
            {/* Charges Table */}
            <TableContainer sx={{ mb: 3, overflow: 'visible' }}>
              <Table size="small">
                <TableHead>
                  <TableRow sx={{ borderBottom: '2px solid #7A4E2D' }}>
                    <TableCell sx={{ textTransform: 'uppercase', fontWeight: 700, color: '#3B2C20', letterSpacing: '0.05em', border: 'none', py: 1.5, pl: 0 }}>Description</TableCell>
                    <TableCell align="right" sx={{ textTransform: 'uppercase', fontWeight: 700, color: '#3B2C20', letterSpacing: '0.05em', border: 'none', py: 1.5, pr: 0 }}>Amount</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
            ...
            {/* Footer / Payment Info */}
            <Box sx={{ bgcolor: '#f9fafb', p: 2, borderRadius: 3, display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' }, gap: 2 }}>
              <Box>
                <Typography variant="caption" sx={{ color: '#3B2C20', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Payment Method</Typography>
                <Box sx={{ display: 'flex', alignItems: 'center', mt: 1, gap: 1 }}>
                  <Chip label={paymentMethod} size="small" sx={{ bgcolor: '#7A4E2D', color: 'white', fontWeight: 600 }} />
        ...
        <Card sx={{ maxWidth: 600, margin: '0 auto', borderRadius: 4, boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
          <CardContent sx={{ p: 4 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 3, gap: 2 }}>
              <Box sx={{ p: 1.5, bgcolor: '#F7F5F2', borderRadius: 2, color: '#7A4E2D' }}>
                <ReceiptLongIcon />
              </Box>
